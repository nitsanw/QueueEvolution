 ...
 */
package psy.lob.saw.queues.mpsc;

import static psy.lob.saw.util.Pow2.findNextPositivePowerOfTwo;
import static psy.lob.saw.util.Pow2.isPowerOf2;
...
abstract class MPSCQueue33ColdFields<E> extends MPSCQueue33L0Pad {
	...
	protected final MPSCQueue31<E>[] queues;

	@SuppressWarnings("unchecked")
	public MPSCQueue33ColdFields(int capacity) {
		queues = new MPSCQueue31[PARALLEL_QUEUES];
		for (int i = 0; i < PARALLEL_QUEUES; i++) {
			queues[i] = new MPSCQueue31<E>(findNextPositivePowerOfTwo(capacity) / PARALLEL_QUEUES);
		}
	}
}
...
public final class MPSCQueue33<E> extends MPSCQueue33L3Pad<E> implements Queue<E> {

	public MPSCQueue33(final int capacity) {
		super(capacity);
	}
