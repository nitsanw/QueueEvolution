 ...
 */
package psy.lob.saw.queues.spsc9;
...
public final class SPSCQueue9<E> extends L3Pad<E> implements Queue<E> {
    ...
    public SPSCQueue9(final int capacity) {
        super(capacity);
    }
    ...
    public int pollBatch(Processor<E> p) {
        long currentHead = getHead();
        if (currentHead >= tailCache) {
            while (currentHead >= (tailCache = getTail())) {
                Thread.yield();
            }
        }
        long lastHead = currentHead;
        do {
            final long offset = ARRAY_BASE + ((currentHead & mask) << ELEMENT_SHIFT);
            @SuppressWarnings("unchecked")
            final E e = (E) UnsafeAccess.UNSAFE.getObject(buffer, offset);
        ...
        headLazySet(currentHead);
        return (int) (currentHead - lastHead);
    }
