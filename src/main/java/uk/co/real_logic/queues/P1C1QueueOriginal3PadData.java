 ...
 * </ul>
 */
public final class P1C1QueueOriginal3PadData<E> implements Queue<E> {
	...
	public P1C1QueueOriginal3PadData(final int capacity) {
		this.capacity = findNextPositivePowerOfTwo(capacity);
		mask = this.capacity - 1;
		buffer = (E[]) new Object[this.capacity+32];
	}
	...
	public boolean offer(final E e) {
		...
		buffer[16 + ((int) currentTail & mask)] = e;
	...
	public E poll() {
		...
		final int index = 16+((int) currentHead & mask);
	...
	public E peek() {
		return buffer[16 + ((int) head.get() & mask)];
	}
	...
	public boolean contains(final Object o) {
		...
		for (long i = head.get(), limit = tail.get(); i < limit; i++) {
			final E e = buffer[16 + ((int) i & mask)];
			if (o.equals(e)) {
				return true;
			}
		}
