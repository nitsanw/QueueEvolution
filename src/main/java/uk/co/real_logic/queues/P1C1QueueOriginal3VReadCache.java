 ...
 * </ul>
 */
public final class P1C1QueueOriginal3VReadCache<E> implements Queue<E> {
	...
	public static final class PaddedLong {
	    private final static long V_OFFSET;
	    static {
	        try {
	            V_OFFSET = UnsafeAccess.UNSAFE.objectFieldOffset(PaddedLong.class.getDeclaredField("value"));
		...
		public long getValueVolatile(){
			return UnsafeAccess.UNSAFE.getLongVolatile(this, V_OFFSET);
		}
	}
	...
	public P1C1QueueOriginal3VReadCache(final int capacity) {
	...
	public boolean offer(final E e) {
		...
		if (headCache.getValueVolatile() <= wrapPoint) {
	...
	public E poll() {
		...
		if (currentHead >= tailCache.getValueVolatile()) {
